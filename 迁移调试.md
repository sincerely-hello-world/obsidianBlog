...
# 规则 1：GPIO 控制器及引脚权限（匹配 /sys/class/gpio 下所有设备）
SUBSYSTEM=="gpio", ACTION=="add", \
  GROUP="gpio", MODE="0660"  # 所有 GPIO 设备归属 gpio 组，组内成员可读写（0660）

# 规则 2：PWM 控制器及通道权限（匹配 /sys/class/pwm 下所有设备）
SUBSYSTEM=="pwm", ACTION=="add", \
  GROUP="pwm", MODE="0660"   # 所有 PWM 设备归属 pwm 组，组内成员可读写（0660）

# 补充：若内核未正确设置 SUBSYSTEM，通过 sysfs 路径强制匹配（兼容性保障）
KERNEL=="gpio*", SUBSYSTEMS=="gpio", ACTION=="add", MODE="0660", GROUP="gpio"
KERNEL=="pwm*", SUBSYSTEMS=="pwm", ACTION=="add", MODE="0660", GROUP="pwm"
