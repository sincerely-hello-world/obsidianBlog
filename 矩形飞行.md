原路径规划
![](assets/矩形飞行/file-20251217173703079.png)


矩形飞行规划
![](assets/矩形飞行/file-20251217162848634.png)
...
//（x为机头朝向（与地面平行），y为朝向机头左方（与地面平行），z为上方）
 ...
 int Fx[] = {275, 275,  25,  25};
 int Fy[] = {25,  375,  375, 25}; 
...
//                if (300 < Mode_Inf->delay_count) // 悬停 6s = 6000ms = 20ms * 300
//                {
...
//                        {// zx_x=detect_x;  动物位置上方x		zx_y=detect_y; //动物位置上方y //在uart2有写
//                            state   = 1; //到达了标记的动物位置
//                            backfly = 0;
//                        }
...
//            }
        ...
        // 以下为位置/高度控制,速度调控
        //-----------------------------------------------------
        if (Mode_Inf->posiyion_lock == true) { //[ ] C 根据状态，执行飞控位置调整
        ...
        if (Mode_Inf->height_lock == true) { //[ ] C 根据状态，执行飞控高度调整
...
Quaternion_getYaw(get_Airframe_attitude());