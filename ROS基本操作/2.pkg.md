## 新建功能包
```bash
ros2 pkg create car  --build-type ament_python  --dependencies rclpy
ros2 pkg create <包名name> --build-type ament_python --node-name my_node --dependencies rclpy

```

ros2 pkg create test_pkgname --build-type ament_python --node-name my_node_name --dependencies rclpy 
## 包的目录结构：
```bash
./my_pkg/
├── my_pkg 
