...
static bool Move_to_XYLine_original(float x, float y, float v);
...
const float MAX_SPEED = 35.0f;      // 水平方向，平移最大速度35   上升最大速度60
...
//							if (gofly == 1) {
//										if (state == 0) {
//												if (Move_to_XYLine_original(zx_x, zx_y, 25) == true) // 距离远 更快， 节省时间；避免过充
...
封装等级：3
*******************************************/
static bool Move_to_XYLine(float x, float y, float v) //self
{
    static uint8_t l_step = 0;
    float delta_x = x - t265_x;
    float delta_y = y - t265_y;
    ...
    else if (l_step == 1)
    {
        ...
        {
            // 远距离保持设定速度
					  now_voly = sign_f(delta_y)* v ;
            //now_voly += 0.2f * (vy - now_voly); // 改动1 x // 可调：0~1，越小越平滑 一阶线性滤波
        }
        else if (fabsf(delta_y) > STOP_DIST)
        {
            // 减速区：速度随距离线性减小
            if(fabsf(now_voly) > 2*MIN_SPEED  )now_voly += 0.3f * (min_vy - now_voly); // 改动1 x
...
static bool Move_to_X(float x, float v, uint8_t clear) // 输入速度v永远是 +++++！
{
    ...
    else if (lx_step == 1)
    {
        ...
        { // 改动1 x
            // 远距离保持设定速度
					  now_volx = sign_f(delta_x)*fmin(v,  MAX_SPEED);
            //now_volx += 0.2f * (vx - now_volx); // 改动1 x // 可调：0~1，越小越平滑 一阶线性滤波
        }
...
static bool Move_to_Y(float y, float v, uint8_t clear) // v永远为+正值输入
{
    ...
    else if (ly_step == 1)
    {
        ...
        {
            // 远距离保持设定速度
						now_voly = sign_f(delta_y)*fmin(v,  MAX_SPEED);
