...
[Service]  
Type=simple  
ExecStart=/home/orangepi/Desktop/yhc-g/start.sh
User=orangepi  
...
cat /etc/systemd/system/muav_car.service  ## 不可行的方案
```bash
[Unit]
Description=uav_car飞行站开机自启服务
...
### 新写的 UAV_CAR__Test/uav_car.sh：使用相对路径：
```bash
#!/bin/bash
...
python3 "$SCRIPT_DIR/src/study4_2.py"  
```
...
# 初始化
echo 'ros2项目构建时所在目录: '$SCRIPT_DIR/UAV_CAR/src
( cd $SCRIPT_DIR/UAV_CAR &&  colcon build )
