...
chown -R root:orangepi /sys/class/gpio  /sys/class/pwm
chmod -R ug+rw  /sys/class/gpio  /sys/class/pwm
...
```bash
SUBSYSTEM=="gpio*", PROGRAM="/bin/sh -c 'chown -R root:gpio /sys/class/gpio && chmod ug+rw  /sys/class/gpio; chown -R root:gpio /sys/devices/virtual/gpio && chmod -R 770 /sys/devices/virtual/gpio'"
```
