...
static bool Move_to_XYLine(float x, float y, float v)
{
    ...
    float distance        = sqrt(delta_x * delta_x + delta_y * delta_y);
		float vx = 0, min_vx = 0;
	  float vy = 0, min_vy = 0;
	  if(delta_x < 0 ) { vx = -v;  min_vx = -MIN_SPEED;}
		if(delta_y < 0 ) { vy = -v;  min_vy = -MIN_SPEED;}
...
//			    now_voly = 0 ;
        l_step++;
				...
				//改动2
						// 根据距离动态调整速度 xxxxxx
						if (fabs(delta_x) > SLOW_DOWN_DIST) {
								// 远距离保持设定速度
									now_volx += 0.2f*(vx - now_volx);  //改动2 x // 可调：0.01~0.1附近，越小越平滑 sign_f(delta_vx)
						} else if (fabs(delta_x) > STOP_DIST) {
								// 减速区：速度随距离线性减小
									now_volx += 0.2f*(min_vx - now_volx);  //改动2  x // 可调：0.01~0.1附近，越小越平滑
						}else{
									// 近距离直接锁定位置
									Mode_Inf->target_x = x;
									now_volx = 0; // 停止移动
						...
						// 根据距离动态调整速度 yyyyyy
						if (fabs(delta_y) > SLOW_DOWN_DIST) {
								// 远距离保持设定速度
									now_voly += 0.2f *(vy - now_voly);  //改动1 x // 可调：0~1，越小越平滑 一阶线性滤波
						} else if (fabs(delta_y) > STOP_DIST) {
								// 减速区：速度随距离线性减小
										now_voly += 0.2f *(min_vy - now_voly);//改动1 x
						}else {
									// 近距离直接锁定位置
									Mode_Inf->target_y = y;
									now_voly = 0; // 停止移动
...
extern bool target_liscence;//孤立的 私有变量，没在.h声明，无其他关联。无用，可以删去。 
extern bool yaw_liscence;
extern bool chongfeixuke;
extern bool huoyuan;//孤立的 私有变量，没在.h声明，无其他关联。无用，可以删去。  //大概是 2024年的送货任务变量？ 但全项目无用到
extern bool chuxianhuoyuan;
extern float yaw_fix, yaw_y;
extern bool landflag;
...
static void M37_Liu_enter()
{
    ...
    Mode_Inf->posiyion_lock = false; // 关闭高度锁和位置锁
...
const float MIN_SPEED      = 4.0f;  // 最小速度限制
 ...
 * - 输出坐标 (display_x, display_y) 范围: display_x∈[25,325], display_y∈[25,425]
 * - (0,0) -> (325, 425)
 * - (6,8) -> (25, 25)
 *
 * @param x 网格x坐标 (行索引)
...
// void init_drv_Uart3(); //在drv_main.c已初始化！此处无需再次调用

//（x为机头朝向（与地面平行），y为朝向机头左方（与地面平行），z为上方）
    // 定义矩形的四个顶点坐标 (从(6,8)到(0,0)的矩形)
//int Fx[] = {  25,   75,  125,   125,  125,  75,   25,   25,   25 };   
//int Fy[] = {  25,   25,   25,    75,  125,  125,  125,  75,   25 };
// 						0     1      2     3    4      5     6     7    8
int Fx[] = {25,250, 250,  25,  25};
...
static void M37_Liu_MainFunc()
{
    ...
    {                                                                        // [ ] A  初始化： PID参数初始化,解锁飞行权
        ...
        {
            if (t265_x != 0 || t265_y != 0 || t265_z != 0) { // 起飞许可
							if (T265_is_ready == 1) { //T265_is_ready 来自 uart7: 收到t265校准确认/起飞指令 <0x52> <空格>， 才被置为1
...
//					  if(Mode_Inf->count < 110 )
//						{
//							Mode_Inf->target_yaw += Quaternion_getYaw(get_Airframe_attitude());
//						}
        } else if (Mode_Inf->count >= 150)     // 20ms*150 = 3000ms = 3s秒后起飞
        {                                      // [ ] A  初始化： 控制授权使能
//					  Mode_Inf->target_yaw = Mode_Inf->target_yaw/110;
//						Lock_yaw_same();
            ALL_Enable();
            Led_setSignal(LED_signal_null);
        }
        switch (Mode_Inf->zt) { //[ ] B 状态切换任务控制
            case 0: {           // XXX   进入起飞流程：初始化
...
//           }
					case 2: { //矩形飞行
													static uint8_t i=0;
													if (Move_to_XYLine(Fx[i], Fy[i], 25) == true)
													{
															++i;
															if (i >= 4)
															{
...
//            }
        ...
        if (Mode_Inf->posiyion_lock == true) { //  悬停在XY，如果有飘逸，P校准归位。
            ...
            if (fabsf(now_voly) < 10) {
                now_voly = delta_y * 2.5;
            } else if (fabsf(now_voly) < 20) {
                now_voly = delta_y * 1.5;
            }
						
            Position_Control_set_TargetVelocityXY(now_volx, now_voly);// 速度值： -45~-20  20~45 合适区间
        }

        if (Mode_Inf->height_lock == true) { // 维持高度
        ...
        l_step++;
				...
				//改动2
						...
						if (fabs(delta_x) > SLOW_DOWN_DIST) {
								...
								// 减速区：速度随距离线性减小
								if(fabs(now_volx) > MIN_SPEED){
									now_volx += 0.1f*(min_vx - now_volx);  //改动2  x // 可调：0.01~0.1附近，越小越平滑
								}
						}else{
									// 近距离直接锁定位置
									Mode_Inf->target_x = x;
						...
						if (fabs(delta_y) > SLOW_DOWN_DIST) {
								...
								// 减速区：速度随距离线性减小
								if(fabs(now_voly) > MIN_SPEED){
										now_voly += 0.1f *(min_vy - now_voly);//改动1 x
								}
						}else {
									// 近距离直接锁定位置
									Mode_Inf->target_y = y;
...
//		const float MAX_ACCEL      = 8.0f;  // 最大加速度限制
    ...
    if (lx_step == 0) {
        ...
        // 根据距离动态调整速度
        if (abs_delta > SLOW_DOWN_DIST) { //改动1 x
            // 远距离保持设定速度
					  if(fabs(now_volx) < fabs(v))  {
							now_volx += 0.2f *(v - now_volx);  //改动1 x // 可调：0~1，越小越平滑 一阶线性滤波
						}
            target_speed = now_volx;
...
//            target_speed      = sign_f(delta_x) * fmaxf(MIN_SPEED, fabs(v) * speed_ratio);//sin函数曲线 变速
        } else {
            // 近距离直接锁定位置
            Mode_Inf->target_x = x;
...
static int8_t Takeoff(float x, float y, float z, float v)
{
    ...
    if (xyz_step == 0) {
        ...
        if (fabsf(delta_x) > 0.7 * v) { // 0.6介稳飞行
        ...
        if (fabsf(delta_y) > 0.7 * v) { // 0.6介稳飞行
