...
```bash
wget http://fishros.com/install -O fishros && . fishros # 后续提示输入密码，解锁权限
```

选择 不换源/ 或者换成 华为源 都可。 (orangepi无需修改，官方镜像源可用)
选择 安装 ros2 Foxy 版本 ！
选择 安装 ros2 Foxy的桌面Desktop版本！ 否则有许多小组件缺失，需要手动调整，麻烦

然后，出错了

2. [参考这篇帖子，手动更新一下 密钥和ros软件](https://fishros.org.cn/forum/topic/4595/%E4%B8%80%E9%94%AE%E5%AE%89%E8%A3%85%E6%8A%A5%E9%94%99404?_=1763952804957)，因为鱼香肉丝安装出现了404 ip not found错误，密钥缺失导致的
```
...
sudo apt install ros-dev-tools -y # 可选不必须， 但必选，因为用的python版本，需要用到colcon构建工具
 ...
 # 只需执行一次，以后新的普通用户 终端都自动有 ROS 2 环境，root用户还需另外配置
echo "source /opt/ros/foxy/setup.bash" >> ~/.bashrc && sudo source ~/.bashrc
```

4. 最后 查看ros版本： printenv | grep -i ROS
```
echo "source /opt/ros/foxy/setup.bash" >> ~/.bashrc && sudo source ~/.bashrc # 只需执行一次，以后新终端都自动有 ROS 2 环境
...
---
 镜像源， 手动或者按照 鱼香ros自动 改也行。(orangepi无需修改，官方镜像源可用！)
 (orangepi无需修改，官方镜像源可用！)
 + 要复制的镜像源配置：Ubuntu20.04(Focal 镜像源， 一定要认准版本号， 一般后面都有 focal字段标识，仅供该版本号ubutnu focal 20.04使用！)
```
# 看一下镜像源：
