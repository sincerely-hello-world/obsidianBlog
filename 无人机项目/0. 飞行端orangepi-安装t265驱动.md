...
mkdir ~/my -p && cd my && pwd
git clone -b v2.51.1 https://github.com/IntelRealSense/librealsense.git  
# 这里放在了 orangepi的 ~/my/librealsense-R-251/ 这里，已经编译并安装好了，剩下步骤可做参考。
...
make -j8
sudo make install
# 遇到 clone <third packages > timeout , 请手动下载 相关库包的<指定版本！>，修改 librealsense/CMake/external_libxxxxx.cmake 改为本地安装
# 比如
# external_libcurl ： 修改 GIT_REPOSITORY "https://github.com/curl/curl.git" 为  URL "/home/orangepi/my/librealsense-R-251/curl.zip"， 必须是下面指定的git tag版本的包


# 将 特殊的python 动态库包加入 环境变量，避免python找不到包/import失败。
...
3D数据图
![](assets/0.%20飞行端orangepi-安装t265驱动/file-20260115191406172.png)

2D数据图
![](assets/0.%20飞行端orangepi-安装t265驱动/file-20260115191406178.png)
