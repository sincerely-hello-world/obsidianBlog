...
# 初始化
echo 'ros2项目初始化bash所在路径: '$ROS_LOG_DIR/UAV_CAR/install/setup.bash
source /opt/ros/foxy/setup.sh
source $ROS_LOG_DIR/UAV_CAR/install/setup.bash # ros2项目启动bash所在路径


# 启动项目
echo 'ros2项目启动python所在路径: '$ROS_LOG_DIR/UAV_CAR/install/uav_car_launch/share/uav_car_launch/launch/
echo 'ros2项目启动python源码所在路径: '$ROS_LOG_DIR/UAV_CAR/src/uav_car_launch/launch/
ros2 launch uav_car_launch uav_car_drone.launch.py
```
...
( cd $SCRIPT_DIR/UAV_CAR &&  colcon build ) # 利用子shell构建，不影响当前工作目录
