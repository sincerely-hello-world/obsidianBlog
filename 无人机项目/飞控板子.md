...
+ 转发动物位置信息，主要是uav_car\image_processor.py节点下的 **send_location(self, data)**， 以及 重置状态 这个指令msg = "99<空格>“ ur.uart_sender4_send(self, msg)
+ 转发所有的规划路径 ：  \uav_car\navigator.py节点的 **fly_path_callback**
+ **fly_path_callback** 是**地面站**发布**全部飞行路径节点**的话题 **fly_path_topic**的回调函数，fly_path_callback位于飞行端，fly_path_topic被飞行站navigator.py节点的**fly_path_sub**订阅，即为self.fly_path_sub = self.create_subscription( Float64MultiArray, "fly_path_topic", self.fly_path_callback, 5)
```python
