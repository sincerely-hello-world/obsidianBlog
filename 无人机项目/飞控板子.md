...
拿 **香橙派飞行端 uart3发送 xyz数据**来说：**uart_sender3_data_topic**这个话题作为中转站
	...
	4. 总结： 并行创建 串口数据的发布者+订阅者， 周期发布传感器数据到话题**uart_sender3_data_topic**，订阅者读话题，然后 从UartSender的订阅回调，串口写数据给**飞控板**！us即为  import uav_car_unit.uart_sender as us， 可以看到在UartSender类中调用串口**serial**方法发送给**飞控板子**
...
## t265_data_topic： T265Data 航迹信息发布（不转发给飞控板！）
...
            if msg.confidence == 3:
                self.pub_data.publish(msg)  # FIXME t265数据 发给 地面站
                t265_date = ur.t265_data_to_uart_std(self, "58", "35", msg) # FIXME 串口 t265数据 转换
                ur.uart_sender3_send(self, t265_date) 
                # FIXME t265数据 发给 uart_sender3_data_topic 
                # FIXME t265数据 发给 uart_sender3_data_topic 
                # FIXME t265数据 发给 uart_sender3_data_topic
