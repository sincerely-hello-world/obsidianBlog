...
+  位于uav_car_unit\camera.py 节点的发布者**camera__data_topic** （不是UDP话题），负责程序间通信，传图给 uav_car\image_processor.py节点下的rknn图像处理订阅者 ，回调sub_camera_callback，进行动物识别，识别到后send_location->调用t265_data_to_uart_std ->然后ur.uart_sender4_send(self, msg) 发布信息给 **uart_sender4_data_topic**， 完成给飞控板转发数据；同时也转发识别后的图像给UDP端口！
