...
SUBSYSTEM=="gpio", KERNEL=="gpiochip*", ACTION=="add", RUN+="/bin/chown  root:gpio /sys%p/active_low /sys%p/edge /sys%p/direction /sys%p/value && /bin/chmod 660 /sys%p/active_low /sys%p/edge /sys%p/direction /sys%p/value"
```

```bash
# Source - https://stackoverflow.com/a/50650398
# Posted by Roman Savrulin, modified by community. See post 'Timeline' for change history
# Retrieved 2026-01-06, License - CC BY-SA 4.0

SUBSYSTEM=="gpio*", PROGRAM="/bin/sh -c 'find -L /sys/class/gpio/ -maxdepth 2 -exec chown root:gpio {} \; -exec chmod ug+rw {} \; || true'"
...
sudo chown -R root:gpio /sys/class/gpio /sys/class/gpio/export /sys/class/gpio/unexport 
sudo chmod ug+rw   /sys/class/gpio/gpio35/* /sys/class/gpio/gpio54/*



sudo chown -R root:gpio /sys/class/gpio*/active_low /sys%p/edge /sys%p/direction /sys%p/value #/sys%p/uevent
sudo chmod  ug+rw     /sys%p/active_low /sys%p/edge /sys%p/direction /sys%p/value #/sys%p/uevent
