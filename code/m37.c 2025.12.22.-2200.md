...
static void M37_Liu_MainFunc()
{
    ...
    {                                                                        // [ ] A  初始化： PID参数初始化,解锁飞行权
        if (fabs(t265_x - 25) < 10 && fabs(t265_y - 25) < 10 && t265_z < 10) // 相对坐标
        {
            ...
            { // 起飞许可
                if (T265_is_ready == 1)
                { // T265_is_ready 来自 uart7: 收到t265校准确认/起飞指令 <0x52> <空格>， 才被置为1
                    Mode_Inf->start_Lock = true;
//                    PID_Init(&xPID, 0.3, 0, 0.05, PID_MAX_I, PID_MAX_OUT, PID_DEAD_ZONE);
//										PID_Init(&yPID, 0.3, 0, 0.05, PID_MAX_I, PID_MAX_OUT, PID_DEAD_ZONE);
                }
            }
        }
    }
...
static bool Move_to_XYLine(float x, float y, float v)
{
    ...
    else if (l_step == 1)
    {
        ...
        {
            // 减速区：速度随距离线性减小
            now_volx += 0.2f * (min_vx - now_volx); // 改动2  x // 可调：0.01~0.2附近，越小越平滑
        }
        else
        {
            // 近距离直接锁定位置
